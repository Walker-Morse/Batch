 ...
 *
 * EventBridge injects only two variables from the S3 event:
 *   S3_BUCKET → detail.bucket.name
 *   S3_KEY    → detail.object.key
 *
 * All other config is derived inside the task from S3_KEY (see parseConfig in main.go):
 *   TENANT_ID      → S3_KEY segment 1  (inbound-raw/{tenant_id}/...)
 *   CLIENT_ID      → S3_KEY segment 2  (inbound-raw/{tenant_id}/{client_id}/...)
 *   FILE_TYPE      → inferred from filename substring (srg310/srg315/srg320 → default SRG310)
 *   CORRELATION_ID → UUID v5 derived deterministically from (bucket + "/" + key)
 *                    Same key always produces the same correlation ID — EventBridge
 *                    retries and manual replays are fully idempotent via Stage 3 dedup.
 *
 * Required S3 key convention: inbound-raw/{tenant_id}/{client_id}/YYYY/MM/DD/filename.ext
 *
