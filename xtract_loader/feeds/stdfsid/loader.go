...
const FeedName = "" // H record field 2 is "FILTEREDSPEND" not "STDFSID"
...
func (l *Loader) processR(ctx context.Context, lineNum int, fields []string) error {
	...
	err := l.pool.QueryRow(ctx, `
		...
		nullFloat(settleAmt), nullStr(spendCatRaw),
	...
	if err != nil {
		...
		_ = l.pool.QueryRow(ctx,
			`SELECT spend_item_sk FROM reporting.fact_spend_item
			 WHERE source_file_name=$1 AND txn_uid=($2::text)::uuid AND log_id=($3::text)::uuid LIMIT 1`,
...
func (l *Loader) processD(ctx context.Context, lineNum int, fields []string) error {
	...
	spendCat   := parser.Field(fields, 6)
	itemAmt    := parser.FieldFloat64(fields, 8)
	itemQty    := parser.FieldInt64(fields, 9)
	taxAmt     := parser.FieldFloat64(fields, 10)
	discAmt    := parser.FieldFloat64(fields, 11)
	isFunded   := parser.Field(fields, 12) == "1"
	fundedAmt  := parser.FieldFloat64(fields, 14)
	...
	_, err := l.pool.Exec(ctx, `
		...
		nullStr(spendCat),
		nullFloat(itemAmt), nullInt(itemQty), nullFloat(taxAmt), nullFloat(discAmt),
		isFunded, nullFloat(fundedAmt),
		l.tenantID,
	)
	if err != nil {
